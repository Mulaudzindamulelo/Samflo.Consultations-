...
export default function SamfloConsultations() {
    ...
    const loadData = async () => {
      try {
        const [txResult, invResult, clientResult, budgetResult, recurResult] = await Promise.all([
          window.storage.get('samflo-transactions'),
          window.storage.get('samflo-invoices'),
          window.storage.get('samflo-clients'),
          window.storage.get('samflo-budgets'),
          window.storage.get('samflo-recurring')
        ]);
    ...
    const generateRecurring = () => {
      recurring.forEach(rec => {
        ...
        while (currentDate <= today) {
          ...
          if (!exists && currentDate.toDateString() !== startDate.toDateString()) {
            const newTx = {
          ...
          if (rec.frequency === 'daily') currentDate.setDate(currentDate.getDate() + 1);
          else if (rec.frequency === 'weekly') currentDate.setDate(currentDate.getDate() + 7);
          else if (rec.frequency === 'monthly') currentDate.setMonth(currentDate.getMonth() + 1);
          else if (rec.frequency === 'yearly') currentDate.setFullYear(currentDate.getFullYear() + 1);
        }
      });
    };
  ...
  useEffect(() => {
    const saveData = async () => {
      try {
        if (transactions.length > 0) await window.storage.set('samflo-transactions', JSON.stringify(transactions));
        if (invoices.length > 0) await window.storage.set('samflo-invoices', JSON.stringify(invoices));
        if (clients.length > 0) await window.storage.set('samflo-clients', JSON.stringify(clients));
        if (budgets.length > 0) await window.storage.set('samflo-budgets', JSON.stringify(budgets));
        if (recurring.length > 0) await window.storage.set('samflo-recurring', JSON.stringify(recurring));
      } catch (error) {
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '1600px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 style={{ fontSize: '2.5rem', color: '#a78bfa', marginBottom: '0.5rem' }}>💼 Samflo Consultations</h1>
            <p style={{ fontSize: '1rem', color: '#94a3b8' }}>Comprehensive Financial Management System</p>
          </div>
          <input
            type="month"
            value={selectedMonth}
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '2rem', marginBottom: '2rem' }}>
              {categoryData.length > 0 && (
                <div className="card">
                  <h3 style={{ marginBottom: '1rem', color: '#f1f5f9' }}>Expense Breakdown</h3>
              ...
              {monthlyData.some(m => m.income > 0 || m.expense > 0) && (
                <div className="card">
                  <h3 style={{ marginBottom: '1rem', color: '#f1f5f9' }}>12-Month Trend</h3>
                  <ResponsiveContainer width="100%" height={300}>
                    <BarChart data={monthlyData}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#475569" />
                      <XAxis dataKey="month" stroke="#94a3b8" style={{ fontSize: '0.8rem' }} />
                      <YAxis stroke="#94a3b8" style={{ fontSize: '0.8rem' }} />
                      <Tooltip formatter={(value) => `R${value.toFixed(2)}`} contentStyle={{ background: '#1a1f3a', border: '1px solid #475569', borderRadius: '8px' }} />
                      <Legend />
            ...
            {/* Budget Alerts */}
            {budgetAlerts.some(b => b.alert) && (
              <div className="card" style={{ marginBottom: '2rem' }}>
                <h3 style={{ marginBottom: '1rem', color: '#ef4444' }}>⚠️ Budget Alerts</h3>
                {budgetAlerts.filter(b => b.alert).map(budget => (
                  <div key={budget.category} className="alert-box">
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                      <span style={{ fontWeight: '600' }}>{budget.category}</span>
                      <span style={{ color: '#ef4444' }}>{budget.percentage.toFixed(0)}% of limit</span>
                    </div>
                    <div style={{ background: '#334155', height: '8px', borderRadius: '4px', overflow: 'hidden' }}>
                      <div style={{ background: '#ef4444', height: '100%', width: `${Math.min(budget.percentage, 100)}%`, transition: 'width 0.3s' }} />
                    </div>
                    <div style={{ fontSize: '0.85rem', color: '#cbd5e1', marginTop: '0.5rem' }}>
                      R{budget.spent.toFixed(2)} spent of R{budget.monthlyLimit.toFixed(2)} limit
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '2rem' }}>
            <div className="card">
              ...
              <form onSubmit={handleAddTransaction} style={{ display: 'grid', gap: '1rem' }}>
                ...
                <input type="text" placeholder="Invoice Number (Optional)" value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '2rem' }}>
            <div className="card">
              ...
              <form onSubmit={handleAddInvoice} style={{ display: 'grid', gap: '1rem' }}>
                ...
                {clients.length > 0 && (
                  <select value={invoiceForm.clientId} onChange={(e) => setInvoiceForm({ ...invoiceForm, clientId: e.target.value })}>
